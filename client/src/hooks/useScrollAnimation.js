...
export const useScrollAnimation = () => {
  ...
  const viewportSettings = {
    once: true,
    margin: isMobile ? "0px 0px -50px 0px" : "-10% 0px -10% 0px", // Higher margin on mobile for early trigger
    amount: isMobile ? 0.01 : 0.2 // Minimal amount on mobile to trigger sooner and avoid "stuck" feeling
  };

  const fadeUp = {
    hidden: { opacity: 0, y: shouldReduceMotion ? 0 : (isMobile ? 10 : 30) },
    visible: { 
      opacity: 1, 
      y: 0,
      transition: { 
        duration: isMobile ? 0.3 : 0.8, // Snappier on mobile
  ...
  const staggerContainer = {
    ...
    visible: {
      opacity: 1,
      transition: {
        staggerChildren: isMobile ? 0.03 : 0.15, // Significantly faster stagger on mobile
  ...
  const slideInRight = {
    hidden: { opacity: 0, x: shouldReduceMotion ? 0 : (isMobile ? 15 : 50) },
    visible: { 
      opacity: 1, 
      x: 0,
