...
import { User, Mail, Lock, Eye, EyeOff, ArrowRight, Sparkles, ShieldCheck } from "lucide-react";
      ...
      <m.div variants={scaleIn} initial="hidden" animate="visible" className="w-full max-w-[480px] relative">
        ...
        <m.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          <Link to="/" className="inline-flex items-center gap-4 group">
             <div className="w-14 h-14 bg-brand-red rounded-2xl flex items-center justify-center text-white shadow-2xl shadow-brand-red/30 transition-transform duration-500 group-hover:rotate-12">
                <Sparkles size={28} />
             </div>
             <h1 className="text-4xl font-heading font-black tracking-tight text-gray-900">Allredz.</h1>
          </Link>
          <div className="mt-8 space-y-2">
             <h2 className="text-3xl font-heading font-black text-gray-900 tracking-tight">Create Account</h2>
             <p className="text-sm text-gray-400 font-medium italic">Join the elite circle of spice connoisseurs</p>
          </div>
        </m.div>
          ...
          <form onSubmit={handleRegister} className="space-y-8">
            <div className="space-y-3">
              ...
              <div className="relative group/input">
                ...
                <input type="text" name="name" placeholder="John Doe" value={form.name} onChange={handleChange} required className="w-full bg-gray-50 border-none rounded-2xl pl-16 pr-6 py-5 text-sm font-bold text-gray-900 placeholder-gray-300 focus:ring-2 focus:ring-brand-red/10 transition-all outline-none" />
              </div>
            </div>
            ...
            <div className="space-y-3">
              <label className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 ml-2">Secret Access Key</label>
              <div className="relative group/input">
                <div className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within/input:text-brand-red transition-colors">
                  <Lock size={18} />
                </div>
            ...
            <button type="submit" disabled={isLoading} className="premium-button w-full bg-brand-red text-white py-6 shadow-2xl hover:bg-gray-900 group h-20">
              {isLoading ? <div className="w-6 h-6 border-2 border-white/20 border-t-white rounded-full animate-spin mx-auto" /> : <span className="flex items-center gap-3 justify-center">Join Circle <ArrowRight size={18} className="ml-3 group-hover:translate-x-2 transition-transform" /></span>}
            </button>
          </form>

          <div className="mt-12 pt-8 border-t border-gray-50 flex flex-col items-center gap-6">
             <Link to="/login" className="inline-flex items-center gap-2 text-brand-red font-black text-xs uppercase tracking-widest hover:gap-4 transition-all">
